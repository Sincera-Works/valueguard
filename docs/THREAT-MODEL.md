...
3. **The only data transmitted off-device is the values statement,
   transmitted once at policy-compile time to a cloud LLM** — whatever
   chat AI the user pastes it into (the app is model-agnostic and holds
   no API key), or the Anthropic API (Sonnet) via the `policy-compiler` CLI.
